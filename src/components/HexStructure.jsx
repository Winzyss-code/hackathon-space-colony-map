...
/**
 * TorusColonyMap — Professional detailed version
 * - Preserves and improves interactivity (pan/zoom, hover, select)
 * - Each sector (1..6) now has its own deterministic, architectural micro-geometry
 *   (explicitly different shapes, sizes, and color accents) so sectors look distinct.
 * - Small elements are placed in a polar-tiled layout (rings x columns) but their
 *   geometry and color vary by sector type (no randomness) so the result looks planned.
 * - Performance: uses simple primitives, limited number of elements, and deterministic math.
 *
 * Drop this file into your React project and import the default export.
 */
...
export default function App({ size = 1000 }) {
  const cx = size / 2;
  const cy = size / 2;
  ...
  // Per-sector microstructure generators (deterministic, distinct styles)
  // Each returns an array of SVG elements placed inside the annular sector.
  function industryFill(cx, cy, r1, r2, start, end) {
    // long horizontal tanks / hangars in rows, colored steel + accent
    const elems = [];
    const rings = 6;
    const cols = 14;
  ...
  function storageFill(cx, cy, r1, r2, start, end) {
    // stacked rectangular storage racks with subtle color variation indicating materials
    const elems = [];
    const rings = 8;
    const cols = 10;
    const ringStep = (r2 - r1) / rings;
  ...
  function housingFill(cx, cy, r1, r2, start, end) {
    // clusters of small rounded houses/buildings with roof colors for livability
    const elems = [];
    const rings = 7;
    const cols = 16;
    const ringStep = (r2 - r1) / rings;
  ...
  function wasteFill(cx, cy, r1, r2, start, end) {
    // circular digesters and processing modules in regular patterns, orange accents
    const elems = [];
    const rings = 6;
    const cols = 12;
    const ringStep = (r2 - r1) / rings;
  ...
  function researchFill(cx, cy, r1, r2, start, end) {
    // node clusters with connecting lines (labs, test rigs) — bluish tones
    const elems = [];
    const rings = 5;
    const cols = 10;
    const ringStep = (r2 - r1) / rings;
    ...
    for (let ri = 0; ri < rings; ri++) {
      ...
      for (let ci = 0; ci < cols; ci++) {
        ...
        // connecting line to inner neighbour
        if (ci < cols - 1) {
  ...
  function energyFill(cx, cy, r1, r2, start, end) {
    // solar panel arrays (rect tile grids) + reactor modules; dark bluish panels
    const elems = [];
    const rings = 6;
    const cols = 12;
    const ringStep = (r2 - r1) / rings;
